...
def render_job_input():
    ...
    if st.session_state.enable_dev_features:
        if st.button("Hannah_Resume", disabled=st.session_state.analysis_confirmed):
        ...
        if st.button("Garrett Resume", disabled=st.session_state.analysis_confirmed):
            st.session_state.using_dev_data = True
            st.session_state.job_rows = [
                {
